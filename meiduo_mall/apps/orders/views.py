    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            try:
                ...
                redis_con = get_redis_connection('carts')
                id_cont = redis_con.hgetall('carts_%s' % user.id)
                selected_ids = redis_con.smembers('selected_%s' % user.id)
                # 选中商品的id
                # selected_dict
                selected_dict = {}
                for id in selected_ids:
                    selected_dict[int(id)] = int(id_cont[id])
